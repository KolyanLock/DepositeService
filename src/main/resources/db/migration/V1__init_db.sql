...
CREATE INDEX idx_user_date_of_birth
    ON "user" (date_of_birth);
...
CREATE INDEX idx_email_data_email
    ON email_data (email);

CREATE INDEX idx_phone_data_phone
    ON phone_data (phone);

-- 3b) fuzzy-поиск по email / phone
CREATE INDEX idx_email_data_email_trgm
    ON email_data
        USING GIN (email gin_trgm_ops);

CREATE INDEX idx_phone_data_phone_trgm
    ON phone_data
